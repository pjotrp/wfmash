...
//Own includes
#include "engine/low-divergence/lde_align_types.hpp"
...
//External includes
#include "engine/low-divergence/wflign/src/lde_wflign.hpp"
...
namespace lde_align
{
  ...
  class Aligner
  {
    private:
      ...
      const lde_align::Parameters &param;
    ...
    public:
       ...
       */
      explicit Aligner(const lde_align::Parameters &p) :
        param(p)
      {
      ...
      inline static void parseMashmapRow(const std::string &mappingRecordLine, MappingBoundaryRow &currentRecord)
      {
        ...
        const vector<string> mm_id_vec = lde_skch::CommonFunc::split(tokens[12], ':');
        // if the estimated identity is missing, avoid assuming too low values
        const float mm_id = lde_skch::is_a_number(mm_id_vec.back()) ? std::stof(mm_id_vec.back()) : lde_skch::fixed::percentage_identity;
        ...
        //Save words into currentRecord
        {
            ...
            currentRecord.strand = (tokens[4] == "+" ? lde_skch::strnd::FWD : lde_skch::strnd::REV);
  ...
  private:
      ...
      void computeAlignments()
      {
          ...
          lde_progress_meter::ProgressMeter progress(total_alignment_length, "[wfmash::lde_align::computeAlignments] aligned");
...
//#define DEBUG true
#ifdef DEBUG
                      std::cerr << "INFO, lde_align::Aligner::computeAlignments, parsing query sequences in file " << fileName << std::endl;
#endif
                      ...
                      lde_seqiter::for_each_seq_in_file(
                          ...
                          [&](const std::string& qSeqId,
                              ...
                              lde_skch::offset_t len = seq->length();
                              // upper-case our input and make sure it's canonical DNA (for WFA)
                              lde_skch::CommonFunc::makeUpperCaseAndValidDNA((char*)seq->c_str(), len);
...
#endif
          progress.finish();
          std::cerr << "[wfmash::lde_align::computeAlignments] "
      ...
      void doAlignment(
...
#ifdef DEBUG
        std::cerr << "INFO, lde_align::Aligner::doAlignment, aligning mashmap record: " << mappingRecordLine << std::endl;
#endif
        ...
        // upper-case our input and make sure it's canonical DNA (for WFA)
        lde_skch::CommonFunc::makeUpperCaseAndValidDNA(ref_seq, got_seq_len);

        // Shift the pointer to the currentRecord.rStartPos position
        ref_seq = ref_seq + head_padding;

        lde_skch::offset_t refLen = currentRecord.rEndPos - currentRecord.rStartPos;
        ...
        lde_skch::offset_t queryLen = currentRecord.qEndPos - currentRecord.qStartPos;
        queryRegion += currentRecord.qStartPos;

        char* queryRegionStrand = new char[queryLen+1];

        if(currentRecord.strand == lde_skch::strnd::FWD) {
          strncpy(queryRegionStrand, queryRegion, queryLen);    //Copy the same string
        } else {
          lde_skch::CommonFunc::reverseComplement(queryRegion, queryRegionStrand, queryLen); //Reverse complement
        }
        ...
        //Compute alignment
#ifdef DEBUG
        std::cerr << "INFO, lde_align::Aligner::doAlignment, WFA execution starting, query region length = " << queryLen
        ...
        lde_wflign::wavefront::WFlign* wflign = new lde_wflign::wavefront::WFlign(
        ...
        wflign->set_output(
                ...
                !param.sam_format,
                param.no_seq_in_sam);
        wflign->lde_wflign_affine_wavefront(
                currentRecord.qId,// + query_name_suffix,
                queryRegionStrand,
                querySize,
                currentRecord.qStartPos,
                queryLen,
                currentRecord.strand != lde_skch::strnd::FWD,
                currentRecord.refId,
                ref_seq,
                ref_size,

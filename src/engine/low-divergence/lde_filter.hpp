...
{
  /**
   * @namespace lde_lde_skch::CommonFunc
   * @brief     Implements routines to filter mappings
   */
  namespace Filter
  {
    /**
     * @namespace lde_lde_skch::filter::query
    ...
    namespace ref
    {
      ...
      struct Helper
      {
         ...
         */
        void refPosDoPlusOne(eventRecord_t &eventRecord, const lde_skch::Sketch &refsketch)
        {
       ...
       */
      template <typename VecIn>
        void filterMappings(VecIn &readMappings, const lde_skch::Sketch &refsketch, uint16_t secondaryToKeep)
        {
          if(readMappings.size() <= 1)
            return;

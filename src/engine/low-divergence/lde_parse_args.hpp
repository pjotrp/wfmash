...
#include "engine/low-divergence/lde_map_stats.hpp"
#include "engine/low-divergence/lde_commonFunc.hpp"

#include "engine/low-divergence/lde_align_parameters.hpp"

#include "engine/low-divergence/lde_temp_file.hpp"
...
void parse_args(int argc,
	...
	if (query_prefix) {
		map_parameters.query_prefix = lde_skch::CommonFunc::split(args::get(query_prefix), ',');
	}
    ...
    map_parameters.referenceSize = lde_skch::CommonFunc::getReferenceSize(map_parameters.refSequences);
    ...
    if (no_filter) {
        map_parameters.filterMode = lde_skch::filter::NONE;
    } else {
        if (one_to_one) {
            map_parameters.filterMode = lde_skch::filter::ONETOONE;
        } else {
            map_parameters.filterMode = lde_skch::filter::MAP;
        }
    }
    ...
    if (!args::get(wfa_score_params).empty()) {
        const std::vector<std::string> params_str = lde_skch::CommonFunc::split(args::get(wfa_score_params), ',');
    ...
    if (!args::get(wfa_patching_score_params).empty()) {
        const std::vector<std::string> params_str = lde_skch::CommonFunc::split(args::get(wfa_patching_score_params), ',');
    ...
    if (!args::get(wflign_score_params).empty()) {
        const std::vector<std::string> params_str = lde_skch::CommonFunc::split(args::get(wflign_score_params), ',');
        if (params_str.size() != 3) {
    ...
    if (wflign_max_mash_dist) {
        if (args::get(wflign_max_mash_dist) <= 0 || args::get(wflign_max_mash_dist) > 1) {
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, max mash distance must be greater than 0 and less than or equal to 1." << std::endl;
            exit(1);
        }
    ...
    if (wflign_min_wavefront_length) {
        if (args::get(wflign_min_wavefront_length) <= 0) {
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, min wavefront length for heuristic WFlign must be greater than 0." << std::endl;
            exit(1);
        }
    ...
    if (wflign_max_distance_threshold) {
        if (args::get(wflign_max_distance_threshold) <= 0) {
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, max distance threshold for heuristic WFlign must be greater than 0." << std::endl;
            exit(1);
        }
    ...
    if (segment_length) {
        ...
        if (s < 100) {
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, minimum segment length is required to be >= 100 bp." << std::endl
    ...
    if (map_pct_identity) {
        if (args::get(map_pct_identity) < 50) {
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, minimum nucleotide identity requirement should be >= 50\%." << std::endl;
            exit(1);
        }
        map_parameters.percentageIdentity = (float) (args::get(map_pct_identity)/100.0); // scale to [0,1]
    } else {
        map_parameters.percentageIdentity = lde_skch::fixed::percentage_identity;
    }
    ...
    //if (spaced_seed_params) {
        ...
        //} else if (foobar.find(':') !=  std::string::npos) {
            //delimeter = ':';
        //} else {
            //std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, wfmash expects either space or : for to seperate spaced seed params." << std::endl;
            //exit(1);
        //}

        //const std::vector<std::string> p = lde_skch::CommonFunc::split(foobar, delimeter);
        //if (p.size() != 4) {
            //std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, there should be four arguments for spaced seeds." << std::endl;
            //exit(1);
        //}
        ...
        //map_parameters.spaced_seed_params = lde_skch::ales_params{seed_weight, seed_count, similarity, region_length};
...
//                std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, high frequency k-mers length and kmerSize parameter are inconsistent." << std::endl;
//                exit(1);
//            }
//
//            map_parameters.high_freq_kmers.insert(kmer);
//        }
//        std::cerr << "[wfmash] INFO, lde_skch::parseandSave, read " << map_parameters.high_freq_kmers.size() << " high frequency kmers." << std::endl;
//    }
    ...
    if (hg_filter_ani_diff)
    {
        ...
        map_parameters.ANIDiff = lde_skch::fixed::ANIDiff;
    }
    ...
    if (hg_filter_conf)
    {
        ...
        map_parameters.ANIDiffConf = lde_skch::fixed::ANIDiffConf;
    }
    ...
    if (approx_mapping) {
        ...
        if (tmp_base) {
            lde_temp_file::set_dir(args::get(tmp_base));
        } else {
            char cwd[PATH_MAX];
            if (getcwd(cwd, sizeof(cwd)) != NULL) {
                lde_temp_file::set_dir(std::string(cwd));
            } else {
                // Handle error: getcwd() failed
                std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, problem in getting the current directory." << std::endl;
                exit(1);
            }
        }
        ...
        if (align_input_paf) {
            ...
            align_parameters.mashmapPafFile = lde_temp_file::create();
        } else {
            map_parameters.outFileName = lde_temp_file::create();
    ...
    if (num_mappings_for_segments) {
        if (args::get(num_mappings_for_segments) > 0) {
            ...
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, the number of mappings to retain for each segment has to be grater than 0." << std::endl;
            exit(1);
        }
    } else {
    ...
    if (num_mappings_for_short_seq) {
        if (args::get(num_mappings_for_short_seq) > 0) {
            ...
            std::cerr << "[wfmash] ERROR, lde_skch::parseandSave, the number of mappings to retain for each sequence shorter than segment length has to be grater than 0." << std::endl;
            exit(1);
        }
    } else {
    ...
    lde_skch::validateInputFiles(map_parameters.querySequences, map_parameters.refSequences);

    lde_temp_file::set_keep_temp(args::get(keep_temp_files));

...
//External includes
#include "engine/low-divergence/lde_argvparser.hpp"
...
namespace lde_skch
{
   ...
   */
  void initCmdParser(lde_CommandLineProcessing::ArgvParser &cmd)
  {
...
'none' disables filtering", 
  ...
  template <typename VEC>
    void parseFileList(std::string &fileToRead, VEC &fileList)
    {
      ...
      if (in.fail())
      {
        std::cerr << "ERROR, lde_skch::parseFileList, Could not open " << fileToRead << std::endl;
        exit(1);
      }
  ...
  void validateInputFile(std::string &fileName)
  {
    ...
    if (in.fail())
    {
      std::cerr << "ERROR, lde_skch::validateInputFile, Could not open " << fileName << std::endl;
      exit(1);
    }
  }
   ...
   */
  void printCmdOptions(lde_skch::Parameters &parameters)
  {
  ...
  void parseandSave(int argc, char** argv, 
      lde_CommandLineProcessing::ArgvParser &cmd, 
      lde_skch::Parameters &parameters)
  {
    ...
    {
      std::cerr << "ERROR, lde_skch::parseandSave, Provide reference file(s)" << std::endl;
      exit(1);
    }
    ...
    //Size of reference
    parameters.referenceSize = lde_skch::CommonFunc::getReferenceSize(parameters.refSequences); 
    str.clear();
    ...
    if(cmd.foundOption("filter_mode"))
    {
      ...
      {
        std::cerr << "ERROR, lde_skch::parseandSave, Invalid option given for filter_mode" << std::endl;
        exit(1);
      };
    ...
    if(cmd.foundOption("segLength"))
    {
      ...
      {
        std::cerr << "ERROR, lde_skch::parseandSave, minimum segment length is required to be >= 100 bp.\n\
    ...
    if(cmd.foundOption("blockLength"))
    {
      ...
      if (parameters.block_length < 0) {
          std::cerr << "[mashmap] ERROR, lde_skch::parseandSave, min block length has to be a float value greater than or equal to 0." << std::endl;
          exit(1);
      }
    } else {
    ...
    if (cmd.foundOption("chainGap")) {
      ...
      str >> l;
      if (l < 0) {
          std::cerr << "[mashmap] ERROR, lde_skch::parseandSave, chain gap has to be a float value greater than or equal to 0." << std::endl;
          exit(1);
      }
    ...
    if (cmd.foundOption("numMappingsForSegment")) {
      ...
      if (n > 0) {
          ...
          std::cerr << "[mashmap] ERROR, lde_skch::parseandSave, the number of mappings to retain for each segment has to be greater than 0." << std::endl;
          exit(1);
      }
    } else {
    ...
    if (cmd.foundOption("numMappingsForShortSeq")) {
      ...
      if (n > 0) {
          ...
          std::cerr << "[mashmap] ERROR, lde_skch::parseandSave, the number of mappings to retain for each sequence shorter than segment length has to be grater than 0." << std::endl;
          exit(1);
      }
    } else {
    ...
    if(cmd.foundOption("perc_identity"))
    {
      ...
      {
        std::cerr << "ERROR, lde_skch::parseandSave, minimum nucleotide identity requirement should be >= 50\%\n" << std::endl;
        exit(1);
      }
    ...
    if (cmd.foundOption("hgFilterAniDiff")) {
      ...
      if (parameters.ANIDiff < 0 ||  parameters.ANIDiff > 100) {
        std::cerr << "ERROR, lde_skch::parseandSave, ANI difference must be between 0 and 100" << std::endl;
        exit(1);
      }
    } else {
      parameters.ANIDiff = lde_skch::fixed::ANIDiff;
    }
    str.clear();
    ...
    if (cmd.foundOption("hgFilterConf")) {
      ...
      if (parameters.ANIDiffConf < 0 ||  parameters.ANIDiffConf > 1) {
        std::cerr << "ERROR, lde_skch::parseandSave, hypergeometric confidence must be between 0 and 1" << std::endl;
        exit(1);
      }
    } else {
      parameters.ANIDiffConf = lde_skch::fixed::ANIDiffConf;
    }
    str.clear();
    ...
    if(cmd.foundOption("sketchSize")) 
    {
      ...
      parameters.sketchSize = lde_skch::Stat::recommendedSketchSize(
            lde_skch::fixed::pval_cutoff, lde_skch::fixed::confidence_interval,

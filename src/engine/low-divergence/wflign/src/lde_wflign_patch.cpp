...
namespace lde_wflign {
    ...
    void encodeOneStep(const char *filename, std::vector<unsigned char> &image, unsigned width, unsigned height) {
        //Encode the image
        unsigned error = lde_lodepng::encode(filename, image, width, height);

        //if there's an error, display it
        if (error) std::cout << "encoder error " << error << ": " << lde_lodepng_error_text(error) << std::endl;
    }
...
bool do_wfa_segment_alignment(
        ...
        const char* query,
        std::vector<lde_rkmh::hash_t>*& query_sketch,
        const uint64_t& query_length,
        const int& j,
        const std::string& target_name,
        const char* target,
        std::vector<lde_rkmh::hash_t>*& target_sketch,
    ...
    if (query_sketch == nullptr) {
        query_sketch = new std::vector<lde_rkmh::hash_t>();
        *query_sketch = lde_rkmh::hash_sequence(
    ...
    if (target_sketch == nullptr) {
        target_sketch = new std::vector<lde_rkmh::hash_t>();
        *target_sketch = lde_rkmh::hash_sequence(
    ...
    const float mash_dist =
            lde_rkmh::compare(*query_sketch, *target_sketch, extend_data->minhash_kmer_size);
    ...
    if (mash_dist > extend_data->max_mash_dist_to_evaluate) {
        ...
        if (aln.ok) {
            ...
            lde_wflign_edit_cigar_copy(*extend_data->wf_aligner,&aln.edit_cigar);
...
/*
// No more necessary
bool hack_cigar(lde_wfa::cigar_t &cigar, const char *query, const char *target,
...
void do_wfa_patch_alignment(
        ...
        const lde_wflign_penalties_t& convex_penalties,
        lde_alignment_t& aln,
...
#ifdef VALIDATE_WFA_WFLIGN
        if (!lde_validate_cigar(wf_aligner->cigar, query, target, query_length,
                    target_length, j, i)) {
    std::cerr << "cigar failure at alignment " << aln.j << " " << aln.i
              << std::endl;
    lde_unpack_display_cigar(wf_aligner->cigar, query, target, query_length,
...
void write_merged_alignment(
        std::ostream &out,
        const std::vector<lde_alignment_t *> &trace,
        lde_wfa::WFAlignerGapAffine2Pieces& wf_aligner,
        const lde_wflign_penalties_t& convex_penalties,
...
#ifdef WFLIGN_DEBUG
    std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] processing traceback"
      << std::endl;
#endif
...
#define MAX_NUM_INDELS_TO_LOOK_AT 2
        ...
        auto patching = [&query, &query_name, &query_length, &query_start,
            ...
            // Head patching
            {
                ...
                if (query_delta > 0 && query_delta < lde_wflign_max_len_minor) {
...
#ifdef VALIDATE_WFA_WFLIGN
                            if (!lde_validate_cigar(wf_aligner_heads->cigar, query_rev.c_str(), target_rev.c_str(), query_rev.size(),
                                        target_rev.size(), 0, 0)) {
                                std::cerr << "cigar failure at head alignment " << std::endl;
                                lde_unpack_display_cigar(wf_aligner_heads->cigar, query_rev.c_str(), target_rev.c_str(), query_rev.size(),
                                                     target_rev.size(), 0, 0);
                                std::cerr << ">query" << std::endl
            ...
            while (q != unpatched.end()) {
                ...
                */
                    if (query_pos >= query_length ||
                        ...
                        std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] "
                ...
                do {
                    ...
                    if ((size_region_to_repatch > 0 ||
                         ...
                         (query_delta > 2 || target_delta > 2) &&
                                 (query_delta < lde_wflign_max_len_major &&
                                  target_delta < lde_wflign_max_len_major) &&
                                 (query_delta < lde_wflign_max_len_minor ||
                                  target_delta < lde_wflign_max_len_minor))) {
...
#ifdef VALIDATE_WFA_WFLIGN
                            if (!lde_validate_cigar(wf_aligner_tails->cigar, query + query_pos, target - target_pointer_shift + target_pos, query_delta,
                                        target_delta_x, 0, 0)) {
                                std::cerr << "cigar failure at head alignment " << std::endl;
                                lde_unpack_display_cigar(wf_aligner_tails->cigar, query + query_pos, target - target_pointer_shift + target_pos, query_delta,
                                                     target_delta_x, 0, 0);
                                std::cerr << ">query" << std::endl
...
#ifdef WFLIGN_DEBUG
            std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] got unsorted "
                 "patched traceback: ";
            for (auto c : patched) {
                std::cerr << c;
            }
...
#ifdef WFLIGN_DEBUG
                std::cerr
                    << "[lde_wflign::lde_wflign_affine_wavefront] copying traceback"
                    << std::endl;
#endif
...
#ifdef VALIDATE_WFA_WFLIGN
                if (!lde_validate_trace(rawv, query, target - target_pointer_shift,
...
#ifdef WFLIGN_DEBUG
                std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] eroding "
                             "traceback at k="
                          << erode_k << std::endl;
#endif
...
#ifdef VALIDATE_WFA_WFLIGN
            if (!lde_validate_trace(erodev, query, target - target_pointer_shift,
...
#ifdef WFLIGN_DEBUG
            std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] normalizing eroded "
                         "traceback"
                      << std::endl;
#endif
...
#ifdef WFLIGN_DEBUG
            std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] got normalized "
                 "eroded traceback: ";
            for (auto c : erodev) {
                std::cerr << c;
            }
...
#ifdef VALIDATE_WFA_WFLIGN
            if (!lde_validate_trace(pre_tracev, query,
...
#ifdef WFLIGN_DEBUG
    std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] got full patched traceback: ";
    for (auto c : tracev) {
        std::cerr << c;
    }
    std::cerr << std::endl;
...
//            std::endl;
    ...
    if (!lde_validate_trace(tracev, query, target - target_pointer_shift,
                    query_length, target_length_mut, query_start,
                    target_start)) {
        std::cerr
...
query_start : query_end)
    ...
    */
    ...
    char *cigarv = lde_alignment_to_cigar(
...
void write_alignment(
    ...
    if (aln.ok) {
        ...
        char *cigar = lde_wfa_alignment_to_cigar(

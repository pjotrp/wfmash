...
wavefront_aligner_attr_t attributes = lde_wavefront_aligner_attr_default;
...
wavefront_aligner_t* const wf_aligner =
    lde_wavefront_aligner_new(strlen(pattern),strlen(text),&attributes);
// Clear & resize
...
// CIGAR
lde_cigar_print_pretty(
    stderr,
    pattern,strlen(pattern),text,strlen(text),
    &wf_aligner->cigar,mm_allocator);
fprintf(stderr,"SCORE: %d \n",
        lde_cigar_score_gap_affine2p(&wf_aligner->cigar,&affine2p_penalties));
// Free
lde_wavefront_aligner_delete(wf_aligner);

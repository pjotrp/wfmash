...
#include "lde_wflign_patch.hpp"


// Namespaces
namespace lde_wflign {
namespace wavefront {
...
void clean_up_sketches(std::vector<std::vector<lde_rkmh::hash_t>*> &sketches) {
...
WFlign::WFlign(
    ...
    const int lde_wflign_mismatch_score,
    const int lde_wflign_gap_opening_score,
    const int lde_wflign_gap_extension_score,
    const float lde_wflign_max_mash_dist,
    const int lde_wflign_min_wavefront_length,
    const int lde_wflign_max_distance_threshold,
    const uint64_t lde_wflign_max_len_major,
    const uint64_t lde_wflign_max_len_minor,
    const int erode_k,
    ...
    this->lde_wflign_mismatch_score = lde_wflign_mismatch_score;
    this->lde_wflign_gap_opening_score = lde_wflign_gap_opening_score;
    this->lde_wflign_gap_extension_score = lde_wflign_gap_extension_score;
    this->lde_wflign_max_mash_dist = lde_wflign_max_mash_dist;
    this->lde_wflign_min_wavefront_length = lde_wflign_min_wavefront_length;
    this->lde_wflign_max_distance_threshold = lde_wflign_max_distance_threshold;
    this->lde_wflign_max_len_major = lde_wflign_max_len_major;
    this->lde_wflign_max_len_minor = lde_wflign_max_len_minor;
...
int wflambda_extend_match(
    ...
    void* arguments) {
    // Extract arguments
    lde_wflign_extend_data_t* extend_data = (lde_wflign_extend_data_t*)arguments;
    // Expand arguments
    const WFlign& lde_wflign = *(extend_data->lde_wflign);
    const uint64_t query_length = lde_wflign.query_length;
    const uint64_t target_length = lde_wflign.target_length;
    ...
    robin_hood::unordered_flat_map<uint64_t,lde_alignment_t*>& alignments = *(extend_data->alignments);
    std::vector<std::vector<lde_rkmh::hash_t>*>& query_sketches = *(extend_data->query_sketches);
    std::vector<std::vector<lde_rkmh::hash_t>*>& target_sketches = *(extend_data->target_sketches);
#ifdef WFA_PNG_TSV_TIMING
    // wfplots
        ...
        if (f != alignments.end()) {
            ...
            const bool alignment_performed =
                    do_wfa_segment_alignment(
                            *lde_wflign.query_name,
                            lde_wflign.query,
                            query_sketches[v],
                            lde_wflign.query_length,
                            query_begin,
                            *lde_wflign.target_name,
                            lde_wflign.target,
                            target_sketches[h],
                            lde_wflign.target_length,
                            target_begin,
...
int wflambda_trace_match(
    robin_hood::unordered_flat_map<uint64_t,lde_alignment_t*>& alignments,
    lde_wfa::WFAlignerGapAffine& wflambda_aligner,
    std::vector<lde_alignment_t*>& trace,
    ...
    for (i=cigar_length-1;i>=0;--i) {
        switch (cigar_ops[i]) {
            ...
            case 'M': {
                ...
                lde_alignment_t* aln = alignments[k];
                trace.push_back(aln);
                aln->keep = true;
                ++num_alignments;
                // Next
...
* WFling align
*/
void WFlign::lde_wflign_affine_wavefront(
    ...
    lde_wflign_penalties_t wfa_affine_penalties;
    ...
    lde_wflign_penalties_t wfa_convex_penalties;
    ...
    if (lde_wflign_max_mash_dist > 0) {
        max_mash_dist_to_evaluate = lde_wflign_max_mash_dist;
    }
    ...
    if (
        ...
        if (aln->ok) {
            ...
            if (!lde_validate_cigar(wf_aligner->cigar, query, target,
                        segment_length_q, segment_length_t, aln.j, aln.i)) {
                std::cerr << "cigar failure at alignment " << aln.j << " "
                << aln.i << std::endl;
                lde_unpack_display_cigar(wf_aligner->cigar, query,
            ...
            lde_wflign_edit_cigar_copy(*wf_aligner,&aln->edit_cigar);

    #ifdef VALIDATE_WFA_WFLIGN
            if (!lde_validate_cigar(aln.edit_cigar, query, target, segment_length_q,
        ...
        write_merged_alignment(
                ...
                *wf_aligner,
                ...
                lde_wflign_max_len_major,
                lde_wflign_max_len_minor,
                erode_k,
                chain_gap,
                max_patching_score,
                MIN_WF_LENGTH,
        ...
        lde_wflign_penalties_t wflambda_affine_penalties;
        if (lde_wflign_mismatch_score > 0 && lde_wflign_gap_opening_score > 0 && lde_wflign_gap_extension_score > 0){
            wflambda_affine_penalties.match = 0;
            wflambda_affine_penalties.mismatch = lde_wflign_mismatch_score;
            wflambda_affine_penalties.gap_opening1 = lde_wflign_gap_opening_score;
            wflambda_affine_penalties.gap_extension1 = lde_wflign_gap_extension_score;
        } else {
            wflambda_affine_penalties.match = 0;
        ...
        if (lde_wflign_max_distance_threshold <= 0) {
            ...
            wflambda_aligner->setHeuristicWFmash(lde_wflign_min_wavefront_length, lde_wflign_max_distance_threshold);
        }
        ...
        lde_wflign_extend_data_t extend_data;
        extend_data.lde_wflign = this;
...
//        extend_data.wfa_affine_penalties = &wfa_affine_penalties;
        ...
        extend_data.max_num_sketches_in_memory = std::ceil(8589934592.0 / (8.0*sizeof(lde_rkmh::hash_t)*mash_sketch_rate*segment_length_to_use*2) );
#ifdef WFA_PNG_TSV_TIMING
        ...
        if (extend_data.emit_png) {
            ...
            auto plot_point = (v_max <= wfplot_max_size && h_max <= wfplot_max_size)
                        ...
                        image.layer_pixel(point.x, point.y, color);
                              ...
                              : [](const algorithms::xy_d_t &point, algorithms::atomic_image_buf_t& image, const algorithms::color_t &color) {
                        lde_wflign::algorithms::wu_calc_wide_line(
                                point, point,
                                color,
                                image);
                    };
            ...
            const std::string filename = *prefix_wavefront_plot_in_png +
                                         query_name + "_" + std::to_string(query_offset) + "_" + std::to_string(query_offset+query_length) + " _ " + (query_is_rev ? "-" : "+") +
                                         "_" + target_name + "_" + std::to_string(target_offset) + "_" + std::to_string(target_offset+target_length) + ".0.lde_wflign.png";
            encodeOneStep(filename.c_str(), bytes, width, height);
        }
#endif
        ...
        std::cerr << "[lde_wflign::lde_wflign_affine_wavefront] alignment score " << score

 ...
 * Wflign Alignment
 */
lde_alignment_t::lde_alignment_t() {
    j = 0;
    i = 0;
    query_length = 0;
    target_length = 0;
...
//void lde_alignment_t::display(void) {
...
//    std::cerr << std::endl;
//}
//bool lde_alignment_t::validate(
//        const char *query,
//        const char *target) {
//    return lde_validate_cigar(edit_cigar,query,target,query_length,target_length,j,i);
//}
void lde_alignment_t::trim_front(int query_trim) {
...
void lde_alignment_t::trim_back(int query_trim) {
    if (query_trim >= query_length) {
        ok = false;
        return;
    }
 ...
 */
lde_trace_pos_t::lde_trace_pos_t(
        const int j,
        const int i,
...
bool lde_trace_pos_t::at_end() {
    return offset == edit_cigar->end_offset;
}
char lde_trace_pos_t::curr() {
    assert(!at_end());
    return edit_cigar->cigar_ops[offset];
}
bool lde_trace_pos_t::equal(lde_trace_pos_t& other) {
    return j == other.j &&
           i == other.i &&
           curr() == 'M' &&
           curr() == other.curr();
}
bool lde_trace_pos_t::assigned() {

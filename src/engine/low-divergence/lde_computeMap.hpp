...
#include "engine/low-divergence/lde_slidingMap.hpp"
#include "engine/low-divergence/lde_MIIteratorL2.hpp"
#include "engine/low-divergence/lde_ThreadPool.hpp"
#include "engine/low-divergence/lde_filter.hpp"

//External includes
#include "engine/low-divergence/lde_seqiter.hpp"
#include "engine/low-divergence/lde_progress.hpp"
#include "engine/low-divergence/lde_map_stats.hpp"
...
namespace lde_skch
{
  ...
  class Map
  {
    ...
    private:
      ...
      const lde_skch::Parameters &param;

      //reference sketch
      const lde_skch::Sketch &refSketch;
    ...
    public:
       ...
       */
      Map(const lde_skch::Parameters &p, const lde_skch::Sketch &refsketch,
          PostProcessResultsFn_t f = nullptr) :
        param(p),
        refSketch(refsketch),
        processMappingResults(f),
        sketchCutoffs(std::min<double>(p.sketchSize, lde_skch::fixed::ss_table_max) + 1, 1),
    ...
    private:
      ...
      void loadPairsFile(const std::string& filename) {
        ...
        if (!in.is_open()) {
          std::cerr << "[mashmap::lde_skch::Map::loadPairsFile] ERROR: cannot open pairs file: " << filename << std::endl;
          exit(1);
        }
        std::string line;
      ...
      void setProbs() 
      {
        ...
        int ss = std::min<double>(param.sketchSize, lde_skch::fixed::ss_table_max);
        ...
        const auto distDiff = [this, &sketchProbs, deltaANI, min_p, ss] (int cmax, int ci) {
          ...
          for (double ymax = 0; ymax <= cmax; ymax++) {
            ...
            double yi_cutoff = deltaANI == 0 ? ymax : (std::floor(lde_skch::Stat::md2j(
                lde_skch::Stat::j2md(ymax / ss, param.kmerSize) + deltaANI, 
                param.kmerSize
            ) * ss));
      ...
      void mapQuery()
      {
        ...
        lde_progress_meter::ProgressMeter progress(total_seq_length, "[mashmap::lde_skch::Map::mapQuery] mapped");

        for(const auto &fileName : param.querySequences)
        {

#ifdef DEBUG
            std::cerr << "[mashmap::lde_skch::Map::mapQuery] mapping reads in " << fileName << std::endl;
#endif
...
//#ifdef DEBUG
							...
							std::cerr << std::endl
									  << "WARNING, lde_skch::Map::mapQuery, read "
									  << seq_name << " of " << len << "bp "
...
//#endif
        ...
        std::cerr << "[mashmap::lde_skch::Map::mapQuery] "
          ...
          bool filter_ref)
      {
        ...
        {
          std::vector<lde_skch::MappingResult> tmpMappings;
          while (subrange_end != unfilteredMappings.end())
          {
            ...
            if (filter_ref)
            {
              lde_skch::Filter::ref::filterMappings(tmpMappings, this->refSketch, n_mappings);
            }
            else
            {
              lde_skch::Filter::query::filterMappings(tmpMappings, n_mappings, param.dropRand);
            }
      ...
      void mapModuleHandleOutput(MapModuleOutput* output,
                                 ...
                                 lde_progress_meter::ProgressMeter& progress)
        {
...
#ifdef ENABLE_TIME_PROFILE_L1_L2
          std::chrono::duration<double> timeSpentL1 = lde_skch::Time::now() - t0;
          auto t1 = lde_skch::Time::now();
#endif
...
#ifdef ENABLE_TIME_PROFILE_L1_L2
          {
            std::chrono::duration<double> timeSpentL2 = lde_skch::Time::now() - t1;
            std::chrono::duration<double> timeSpentMappingFragment = lde_skch::Time::now() - t0;
...
#ifdef DEBUG
          std::cerr << "INFO, lde_skch::Map::getSeedHits, read id " << Q.seqCounter << ", minmer count = " << Q.minmerTableQuery.size() << ", bad minmers = " << orig_len - Q.sketchSize << "\n";
#endif
        } 
...
#ifdef DEBUG
          std::cerr<< "INFO, lde_skch::Map::getSeedHits, read id " << Q.seqCounter << ", minmer count = " << Q.minmerTableQuery.size() << " " << Q.len << "\n";
#endif
        ...
        void doL2Mapping(Q_Info &Q, L1_Iter l1_begin, L1_Iter l1_end, VecOut &l2Mappings)
        {
          ...
          while (loc_iterator != l1_end)
          {
            ...
            for (auto& l2 : l2_vec) 
            {
              ...
              float nucIdentityUpperBound = 1 - Stat::md_lower_bound(mash_dist, Q.sketchSize, param.kmerSize, lde_skch::fixed::confidence_interval);
      ...
      template <typename Q_Info, typename Vec>
        ...
        {
#ifdef DEBUG
          //std::cerr << "INFO, lde_skch::Map:computeL2MappedRegions, read id " << Q.seqName << "_" << Q.startPos << std::endl; 
#endif
          ...
          constexpr auto heap_cmp = [](const lde_skch::MinmerInfo& l, const lde_skch::MinmerInfo& r) {return l.wpos_end > r.wpos_end;};

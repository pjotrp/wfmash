...
$ mashmap-align -s ref.fa -q seq.fq --mappingFile mashmap.out --pi 80 [OPTIONS]");
   ...
   */
  void printCmdOptions(lde_align::Parameters &parameters)
  {
  ...
  void parseandSave(int argc, char** argv, 
      ...
      lde_align::Parameters &parameters)
  {
    ...
    {
      std::cerr << "ERROR, lde_align::parseandSave, Provide reference file(s)\n\
    ...
    {
      std::cerr << "ERROR, lde_align::parseandSave, Provide query file(s)\n\
        This input should be same as used for generating mashmap mapping output" << std::endl;
      exit(1);
    }
    ...
    else //list of files
    {
      ...
      lde_skch::parseFileList(listFile, parameters.refSequences);
    }
    ...
    else //list of files
    {
      ...
      lde_skch::parseFileList(listFile, parameters.querySequences);
    }
    ...
    lde_skch::validateInputFile(parameters.mashmapPafFile);
    lde_skch::validateInputFiles(parameters.querySequences, parameters.refSequences);
  }
}

...
//External includes
#include "engine/low-divergence/lde_murmur3.h"
#include "common/prettyprint.hpp"
#include "csv.h"

//#include "engine/low-divergence/deps/sparsehash/dense_hash_map"
  ...
  class Sketch
    {
       ...
       */
      Sketch(const lde_skch::Parameters &p) 
        :
          param(p) {
            this->build();
            this->index();
      ...
      void build()
      {
...
#ifdef DEBUG
        std::cerr << "[mashmap::lde_skch::Sketch::build] building minmer index for " << fileName << std::endl;
#endif
        ...
        lde_seqiter::for_each_seq_in_file(
            ...
            [&](const std::string& seq_name,
                ...
                {
#ifdef DEBUG
                    std::cerr << "WARNING, lde_skch::Sketch::build, found an unusually short sequence relative to kmer" << std::endl;
#endif
                }
                else
                {
        ...
        if (seqCounter == 0)
        {
          std::cerr << "[mashmap::lde_skch::Sketch::build] ERROR: No sequences indexed!" << std::endl;
          exit(1);
        }
        ...
        std::cerr << "[mashmap::lde_skch::Sketch::build] minmer windows picked from reference = " << minmerIndex.size() << std::endl;
      ...
      MI_Type* buildHelper(InputSeqContainer *input)
      {
        ...
        lde_skch::CommonFunc::addMinmers(
                *thread_output, 
                &(input->seq[0u]), 
                input->len, 
                param.kmerSize, 
      ...
      void index()
      {
        ...
        std::cerr << "[mashmap::lde_skch::Sketch::index] unique minmers = " << nUniqueMinmers << std::endl;
      }
      ...
      void computeFreqHist()
      {
          if (packed_ok ? !posLookupCSR.empty() : !minmerPosLookupIndex.empty()) {
              ...
              std::cerr << "[mashmap::lde_skch::Sketch::computeFreqHist] Frequency histogram of minmer interval points = "
              ...
              if (this->freqThreshold != std::numeric_limits<int>::max())
                  std::cerr << "[mashmap::lde_skch::Sketch::computeFreqHist] With threshold " << this->param.kmer_pct_threshold
              ...
              std::cerr << "[mashmap::lde_skch::Sketch::computeFreqHist] No minmers." << std::endl;
          }
      }

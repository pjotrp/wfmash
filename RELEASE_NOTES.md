...
* Tracked the release version in a top-level `VERSION` file (0.24.2) and made it visible again: the version header generated by scripts/generate_git_version.sh used to come out empty whenever `git describe` was unavailable at configure time (no git on the PATH, tarball builds), so `--version`, the help header and the SAM `@PG VN:` field carried an empty string. The script now reads the base version from `VERSION` and emits e.g. `0.24.2-16-g<sha>` when git is available, plain `0.24.2` when it is not; the help screen shows `wfmash 0.24.2-16-g<sha> - a pangenome-scale aligner`
...
* Bumped `actions/checkout` from v2 to v5 to clear the Node.js 20 deprecation warnings on current runners
